...
const LineChartWithInteraction = ({ data, labels }) => {
  const maxValue = 10;
  ...
  const handleTouch = (evt) => {
    ...
    const index = Math.round((touchX / (graphWidth - 2 * margin)) * (data.length - 1));
  ...
  const points = data
    .map((value, index) => {
      if (value !== null) {
        const x = (index / (data.length - 1)) * (graphWidth - 2 * margin) + margin;
        ...
        <G>
          {labels.map((label, index) => (
            data[index] !== null && (
              <G key={index}>
                <Line
                  x1={(index / (labels.length - 1)) * (graphWidth - 2 * margin) + margin}
                  y1={margin}
                  x2={(index / (labels.length - 1)) * (graphWidth - 2 * margin) + margin}
                  y2={graphHeight + margin}
                  stroke="black"
                  strokeDasharray="4 4"
                />
                <SvgText
                  x={(index / (labels.length - 1)) * (graphWidth - 2 * margin) + margin}
          ...
          {linePath && <Path d={linePath} fill="none" stroke="#5271FF" strokeWidth="2" />}
          {points.map((point, index) => (
            <Circle key={index} cx={point.x} cy={point.y} r="4" fill="rgba(0, 0, 255, 0.5)" />
          ))}
          {selectedPoint !== null && points[selectedPoint] && (
            <G>
              <Circle cx={points[selectedPoint].x} cy={points[selectedPoint].y} r="6" fill="white" stroke="#5271FF" strokeWidth="2" />
              <Rect
